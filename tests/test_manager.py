...
from conftest import test_preset_file_name, test_resources, test_folder_structure_path, test_preset_tofix_file_name
...
class TestPresetImporting(ODKSMTest):
    ...
    def test_should_handle_broken_mod_names(self):
        """The preset mechanism should handle broken mod names"""
        mods_name = self.manager._parse_mods_preset(
            test_preset_tofix_file_name)
        assert "ACE Compat - RHS- GREF" in mods_name
        assert "F-A-18 Hornet" in mods_name
...
class TestThePresetManager:
    ...
    def test_should_be_able_to_read_a_config_file(self):
        ...
        # this check that empty list field in config don't pollute the list
        assert len(sm.settings.skip_keys) == 1

    def test_should_handle_a_missing_enabled_fixes_in_the_mod_fix_settings_section(self, patch_with_hook):
        ...
        sm = ServerManager()
        sm.config_file = join(test_resources, "config.ini")
        # simulate a missing 'enabled_fixes' keyword in the read_file output
        patch_with_hook(function_to_mock=ConfigIni.read_file,
                        function_to_mock_name="odk_servermanager.manager.ConfigIni.read_file",
                        function_hook=hook)
        sm._parse_config()
    ...
    def test_should_correctly_modify_mods_to_be_copied_from_the_registered_fix(self, mocker):
        ...
        linked = self._prepare_mod_fix_with_dummy_hook(
            "ODKMIN", "hook_init_link_replace")
        not_there = self._prepare_mod_fix_with_dummy_hook(
            "NotThere", "hook_init_copied_replace")
        copied = self._prepare_mod_fix_with_dummy_hook(
            "CBA_A3", "hook_init_copy_replace")
        mocker.patch("odk_servermanager.modfix.register_fixes",
                     side_effect=lambda x: [linked, not_there, copied])
        sm = ServerManager()
        sm.config_file = join(test_resources, "config.ini")
        sm._parse_config()
...
class TestAServerManagerAtInit(ODKSMTest):
    ...
    def test_should_init_the_server_instance(self, reset_folder_structure, mocker):
        ...
        answer = mocker.patch(
            "builtins.input", return_value="y")  # this skips the user input check
    ...
    def test_should_quit_if_not_confirmed_at_init(self, reset_folder_structure, mocker):
        ...
        answer = mocker.patch(
            "builtins.input", return_value="n")  # this fails the user input check
        abort = mocker.patch(
            "odk_servermanager.manager.ServerManager._ui_abort", side_effect=self.sm._ui_abort)
    ...
    def test_should_ask_confirmation_before_update_if_called_twice(self, reset_folder_structure, mocker):
        ...
        update = mocker.patch(
            "odk_servermanager.instance.ServerInstance.update", side_effect=self.sm.instance.update)
        self.sm.manage_instance(self.config_file)
        answer.assert_called()
    ...
    def test_should_catch_exception_regarding_mod_fixes(self, reset_folder_structure, mocker):
        ...
        mocker.patch("odk_servermanager.modfix.register_fixes",
                     side_effect=lambda x: broken_fixes(MisconfiguredModFix))
        self._assert_aborting(self.sm.manage_instance, {
                              "config_file": self.config_file})
        mocker.patch("odk_servermanager.modfix.register_fixes",
                     side_effect=lambda x: broken_fixes(NonExistingFixFile))
        self._assert_aborting(self.sm.manage_instance, {
                              "config_file": self.config_file})
    ...
    def test_should_abort_with_more_general_errors(self, reset_folder_structure, mocker):
        ...
        def broken_stuff():
            raise Exception
        mocker.patch(
            "odk_servermanager.manager.ServerManager._ui_init", side_effect=broken_stuff)
        self._assert_aborting(self.sm.manage_instance, {
                              "config_file": self.config_file})
...
class TestWhenBootstrapping(ODKSMTest):
    ...
    def test_it_should_abort_if_there_are_missing_fields_in_the_default_config_ini(self, mocker):
        ...
        self._assert_aborting(self.sm.bootstrap, {
                              "default_config_file": self.default_file})
        mocker.patch("odk_servermanager.manager.ConfigIni.read_file",
...
class TestWhenDebugging:
    ...
    def test_ui_abort_should_call_print_debug_log_if_needed(self, reset_folder_structure, mocker):
        ...
        print_logs_fun = mocker.patch(
            "odk_servermanager.manager.ServerManager._print_debug_log")
        manager = ServerManager()
